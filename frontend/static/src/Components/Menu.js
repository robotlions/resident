...
doAdd(){
  ...
  render() {
    ...
    const menuList = this.state.data.map((data) =>
      this.state.isEditing === true && this.state.editWindow === data.id ? (
        <form key={data.id}>
          ...
          <br />
          Image:
          <input type="file" className="input-window" name="image" onChange={this.handleImage} />
          {this.state.image && (
    ...
    return <div><div className="row justify-content-center">{menuList}</div>
    <div>
    {this.state.isLoggedIn && this.state.isAdding === false ? addButton : null}
    {this.state.isLoggedIn && this.state.isAdding === true ? addItem : null}
    </div>
    </div>;
  }
}
